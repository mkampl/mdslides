...
MarkdownSlideRenderer::MarkdownSlideRenderer() 
    : current_slide(0), show_timer(false), utf8_supported(false) {
    setup_unicode_support();
}

void MarkdownSlideRenderer::setup_unicode_support() {
    ...
    // Load character replacements for ASCII fallback
    char_replacements = {
        // German umlauts
...
void MarkdownSlideRenderer::safe_mvprintw(int y, int x, const std::string& text) {
...
void MarkdownSlideRenderer::render_element_animated(const SlideElement& element) {
...
void MarkdownSlideRenderer::render_slide_animated() {
...
void MarkdownSlideRenderer::render_slide_instant() {
    clear_with_background();
    draw_header();
    draw_footer();
...
void MarkdownSlideRenderer::update_shell_output_display(const SlideElement& shell_element) {
...
void MarkdownSlideRenderer::draw_header() {
    attron(COLOR_PAIR(1) | A_BOLD);
    mvprintw(0, 2, "Slide %d/%d", current_slide + 1, slides.get_slide_count());
    ...
    mvprintw(0, COLS - 15, "Theme: %s", theme_manager.get_current_theme_name());
...
void MarkdownSlideRenderer::draw_footer() {
    attron(COLOR_PAIR(4));
...
void MarkdownSlideRenderer::draw_progress_bar() {
    if (slides.is_empty()) return;
    
    int progress_width = (current_slide * (COLS - 4)) / slides.get_slide_count();
...
void MarkdownSlideRenderer::show_help() {
    clear_with_background();
...
void MarkdownSlideRenderer::clear_with_background() {
...
void MarkdownSlideRenderer::goto_slide() {
    ...
    mvprintw(LINES/2, 2, "Go to slide (1-%d): ", slides.get_slide_count());
